...
class ConvolutionTest(tf.test.TestCase):
  ...
  def testCreateConv(self):
    ...
    with self.cached_session():
      ...
      weights = slim.get_variables_by_name('weights')[0]
      self.assertListEqual(weights.get_shape().as_list(), [3, 3, 4, 32])
      biases = slim.get_variables_by_name('biases')[0]
  ...
  def testCreateConvCreatesWeightsAndBiasesVars(self):
    ...
    images = tf.random.uniform((5, height, width, 3), seed=1)
    with self.cached_session():
      self.assertFalse(slim.get_variables('conv1/weights'))
      self.assertFalse(slim.get_variables('conv1/biases'))
      conv2d_ws.conv2d(images, 32, [3, 3], scope='conv1')
      self.assertTrue(slim.get_variables('conv1/weights'))
      self.assertTrue(slim.get_variables('conv1/biases'))
  ...
  def testCreateConvWithWD(self):
    ...
    with self.cached_session() as sess:
      ...
      l2_loss = tf.nn.l2_loss(
          slim.get_variables_by_name('weights')[0])
  ...
  def testNonReuseVars(self):
    ...
    with self.cached_session():
      ...
      self.assertEqual(len(slim.get_variables()), 2)
      conv2d_ws.conv2d(images, 32, [3, 3])
      self.assertEqual(len(slim.get_variables()), 4)

  def testReuseConvWithWD(self):
    height, width = 7, 9
    with self.cached_session():
      images = tf.random.uniform((5, height, width, 3), seed=1)
      weight_decay = slim.l2_regularizer(0.01)
  ...
  def testConvWithBatchNorm(self):
    ...
    with self.cached_session():
      ...
      self.assertEqual(len(slim.get_variables()), 8)
      self.assertEqual(
          len(slim.get_variables('Conv/BatchNorm')), 3)
      self.assertEqual(
          len(slim.get_variables('Conv_1/BatchNorm')), 3)
  ...
  def testReuseConvWithBatchNorm(self):
    ...
    with self.cached_session():
      ...
      self.assertEqual(
          len(slim.get_variables('Conv/BatchNorm')), 3)
      self.assertEqual(
          len(slim.get_variables('Conv_1/BatchNorm')), 0)
